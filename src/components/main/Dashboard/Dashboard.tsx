...
export const Dashboard = ({ currentView }: Props) => {
  ...
  return (
    <div className="sm:main-background xs mx-auto mt-4 w-[calc(100%-1rem)] max-w-[280px] rounded-lg p-4 xs:w-[calc(100%-2rem)] xs:max-w-[360px] sm:w-full sm:max-w-[640px] sm:border-2 sm:border-mainColor sm:p-6 md:p-8 lg:mt-8 lg:max-w-[720px]">
      <div className="mb-8">
        <NiccaMessage className="dashboard-component relative flex h-full items-center border-2 border-mainColor bg-[#ECEFF4] p-6" />
      </div>
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
        <div className="h-full w-full">
          <CustomCalendar className="dashboard-component h-full w-full bg-[#ECEFF4]" />
        </div>
        <div className="dashboard-component flex h-full flex-col justify-between border-2 border-mainColor bg-[#ECEFF4] p-6">
      ...
      <div className="mt-8">
        <AchievementMetrics className="dashboard-component bg-[#ECEFF4]" />
      </div>
    </div>
  );
};
